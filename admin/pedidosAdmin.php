...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold text-center mb-8 text-white">Gestión de <span class="text-violet-700">Pedidos</span></h1>

    <?php foreach ($pedidos_por_estado as $estado => $pedidos): ?>
    <h1 class="text-xl text-violet-700 font-bold mb-4 capitalize"><?= ucfirst($estado) ?></h1>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
        <?php foreach ($pedidos as $pedido): ?>
        <div class="relative bg-white rounded-lg shadow-md p-4 border border-gray-200">
            <h3 class="font-bold text-lg mb-2 text-violet-700">Pedido #<?= $pedido['id'] ?></h3>
            ...
            <div class="mt-4">
                ...
                <select class="select select-bordered w-full mt-2 text-black"
            ...
            <div class="mt-4 flex justify-end">
                <button onclick="eliminarPedido(<?= $pedido['id'] ?>)" class="btn font-bold bg-violet-700 text-white btn-sm hover:bg-violet-900">Eliminar</button>
            </div>
        </div>
        <?php endforeach; ?>
    </div>
